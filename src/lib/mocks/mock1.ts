export const mock1Questions = [
  {
    id: 1,
    text: "What are three responsibilities of citizenship?",
    options: [
      "Being loyal to Canada, recycling newspapers, serving in the navy, army or air force.",
      "Obeying the law, taking responsibility for oneself and one’s family, serving on a jury.",
      "Learning both official languages, voting in elections, belonging to a union.",
      "Buying Canadian products, owning your own business, using less water."
    ],
    correctAnswer: 1,
    category: "Rights and Responsibilities",
    explanation: "The Discover Canada guide outlines responsibilities including: obeying the law, taking responsibility for oneself and one's family, serving on a jury, voting in elections, helping others in the community, and protecting and enjoying our heritage and environment.",
    reference: "Discover Canada Guide, Page 9"
  },
  {
    id: 2,
    text: "What is the meaning of the Remembrance Day poppy?",
    options: [
      "To remember our Sovereign, Queen Elizabeth II.",
      "To celebrate Confederation.",
      "To honour prime ministers who have died.",
      "To remember the sacrifice of Canadians who have served or died in wars up to the present day."
    ],
    correctAnswer: 3,
    category: "Canadian Symbols",
    explanation: "Canadians wear the red poppy to remember the sacrifices of Canadians who have served or died in conflicts up to the present day.",
    reference: "Discover Canada Guide, Page 22"
  },
  {
    id: 3,
    text: "How are Members of Parliament chosen?",
    options: [
      "They are appointed by the United Nations.",
      "They are chosen by the provincial premiers.",
      "They are elected by voters in their local constituency (riding).",
      "They are elected by landowners and police chiefs."
    ],
    correctAnswer: 2,
    category: "How Canadians Govern Themselves",
    explanation: "Members of Parliament (MPs) are elected by Canadian citizens in their local constituencies (ridings) to represent them in the House of Commons.",
    reference: "Discover Canada Guide, Page 29"
  },
  {
    id: 4,
    text: "Name two key documents that contain our rights and freedoms.",
    options: [
      "The Magna Carta and the Canadian Charter of Rights and Freedoms.",
      "The British North America Act and the Treaty of Versailles.",
      "The Declaration of Independence and the Constitution Act.",
      "The Criminal Code and the Civil Code."
    ],
    correctAnswer: 0,
    category: "Rights and Responsibilities",
    explanation: "The Magna Carta (signed in 1215) is a historic document that influences our rights, and the Canadian Charter of Rights and Freedoms (1982) is entrenched in our Constitution.",
    reference: "Discover Canada Guide, Page 9"
  },
  {
    id: 5,
    text: "Name four (4) fundamental freedoms that Canadians enjoy.",
    options: [
      "Freedom to travel, freedom to bear arms, freedom of education, freedom to vote.",
      "Freedom to work anywhere, freedom to own property, freedom of mobility, freedom of speech.",
      "Freedom of conscience and religion; freedom of thought, belief, opinion and expression; freedom of peaceful assembly; and freedom of association.",
      "Freedom from discrimination, freedom of equality, freedom of multiculturalism, freedom of bilingualism."
    ],
    correctAnswer: 2,
    category: "Rights and Responsibilities",
    explanation: "Canadians enjoy fundamental freedoms including conscience and religion; thought, belief, opinion and expression (including free speech and a free press); peaceful assembly; and association.",
    reference: "Discover Canada Guide, Page 9"
  },
  {
    id: 6,
    text: "What is meant by the equality of women and men?",
    options: [
      "Men and women must have the same jobs.",
      "In Canada, men and women are equal under the law.",
      "Women have more rights than men in certain provinces.",
      "Men and women must earn the same salary by law in all private sectors."
    ],
    correctAnswer: 1,
    category: "Rights and Responsibilities",
    explanation: "In Canada, men and women are equal under the law. Canada’s openness and generosity do not extend to barbaric cultural practices that tolerate spousal abuse, honour killings, female genital mutilation, or other gender-based violence.",
    reference: "Discover Canada Guide, Page 9"
  },
  {
    id: 7,
    text: "Who were the founding peoples of Canada?",
    options: [
      "The Americans, the British, and the French.",
      "The Aboriginal peoples, the French, and the British.",
      "The Inuit, the Métis, and the First Nations.",
      "The Vikings, the French, and the First Nations."
    ],
    correctAnswer: 1,
    category: "Who We Are",
    explanation: "Aboriginal, French, and British peoples are the three founding peoples of Canada.",
    reference: "Discover Canada Guide, Page 11"
  },
  {
    id: 8,
    text: "Who are the Métis?",
    options: [
      "A distinct people of mixed Aboriginal and European ancestry.",
      "First Nations people living in the Maritimes.",
      "Inuit who moved to urban centers.",
      "French settlers who arrived in the 1700s."
    ],
    correctAnswer: 0,
    category: "Who We Are",
    explanation: "The Métis are a distinct people of mixed Aboriginal and European ancestry, the majority of whom live in the Prairie provinces.",
    reference: "Discover Canada Guide, Page 11"
  },
  {
    id: 9,
    text: "What does the word “Inuit” mean?",
    options: [
      "The North.",
      "The First.",
      "The Hunters.",
      "The People."
    ],
    correctAnswer: 3,
    category: "Who We Are",
    explanation: "The term Inuit means 'the people' in the Inuktitut language. They live in small, scattered communities across the Arctic.",
    reference: "Discover Canada Guide, Page 11"
  },
  {
    id: 10,
    text: "What is meant by the term “responsible government”?",
    options: [
      "The government must balance the budget every year.",
      "The ministers of the Crown must have the support of a majority of the elected representatives in order to govern.",
      "The Prime Minister must answer all questions from the citizens.",
      "The government cannot be sued by the public."
    ],
    correctAnswer: 1,
    category: "Canada's History",
    explanation: "Responsible government means that the ministers of the Crown must have the support of a majority of the elected representatives in order to govern. If they lose a confidence vote, they must resign.",
    reference: "Discover Canada Guide, Page 19"
  },
  {
    id: 11,
    text: "Who was Sir Louis-Hippolyte La Fontaine?",
    options: [
      "The first Prime Minister of Canada.",
      "A champion of French language rights and the first leader of a responsible government in the Canadas.",
      "The English General who won the Battle of the Plains of Abraham.",
      "A famous French explorer who discovered the St. Lawrence River."
    ],
    correctAnswer: 1,
    category: "Canada's History",
    explanation: "La Fontaine was a champion of French language rights and became the first leader of a responsible government in the Canadas in 1849.",
    reference: "Discover Canada Guide, Page 19"
  },
  {
    id: 12,
    text: "What did the Canadian Pacific Railway symbolize?",
    options: [
      "The industrial revolution in North America.",
      "Unity, joining the East to the West by train.",
      "The economic dominance of the British Empire.",
      "A fast way to move troops during wartime."
    ],
    correctAnswer: 1,
    category: "Canada's History",
    explanation: "A powerful symbol of unity, the Canadian Pacific Railway joined the East to the West, fulfilling a promise made to British Columbia when it joined Confederation.",
    reference: "Discover Canada Guide, Page 20"
  },
  {
    id: 13,
    text: "What does Confederation mean?",
    options: [
      "The separation of Canada from Great Britain.",
      "The joining of provinces to make a new country.",
      "The creation of a free trade zone in North America.",
      "The signing of peace treaties with the First Nations."
    ],
    correctAnswer: 1,
    category: "Canada's History",
    explanation: "Confederation (1867) is the joining of provinces (Ontario, Quebec, Nova Scotia, and New Brunswick) to make a new country called the Dominion of Canada.",
    reference: "Discover Canada Guide, Page 19"
  },
  {
    id: 14,
    text: "What is the significance of the discovery of insulin by Sir Frederick Banting and Charles Best?",
    options: [
      "It cured tuberculosis.",
      "It became a popular export to Europe.",
      "It has saved 16 million lives worldwide and treats diabetes.",
      "It was the first vaccine ever created."
    ],
    correctAnswer: 2,
    category: "Modern Canada",
    explanation: "In 1922, Sir Frederick Banting and Charles Best discovered insulin, a hormone to treat diabetes that has saved 16 million lives worldwide.",
    reference: "Discover Canada Guide, Page 26"
  },
  {
    id: 15,
    text: "What does it mean to say that Canada is a constitutional monarchy?",
    options: [
      "Canada has no written constitution but relies entirely on the British monarchy.",
      "The Prime Minister functions as the King or Queen of Canada.",
      "The Sovereign reigns, but the government rules under the Constitution.",
      "The Constitution was written by a King."
    ],
    correctAnswer: 2,
    category: "How Canadians Govern Themselves",
    explanation: "In a constitutional monarchy, Canada's Head of State is a hereditary Sovereign (Queen or King) who reigns in accordance with the Constitution, while the Head of Government is the Prime Minister.",
    reference: "Discover Canada Guide, Page 29"
  },
  {
    id: 16,
    text: "What are the three branches of government?",
    options: [
      "Federal, Provincial, Municipal.",
      "Executive, Legislative, Judicial.",
      "Monarch, Senate, House of Commons.",
      "Police, Military, Government."
    ],
    correctAnswer: 1,
    category: "How Canadians Govern Themselves",
    explanation: "The three branches of government are the Executive, Legislative, and Judicial branches.",
    reference: "Discover Canada Guide, Page 29"
  },
  {
    id: 17,
    text: "What is the highest honour that Canadians can receive?",
    options: [
      "The Order of Canada.",
      "The Nobel Peace Prize.",
      "The Victoria Cross.",
      "The Medal of Bravery."
    ],
    correctAnswer: 2,
    category: "Canada's History",
    explanation: "The Victoria Cross (V.C.) is the highest honour available to Canadians and is awarded for the most conspicuous bravery.",
    reference: "Discover Canada Guide, Page 40"
  },
  {
    id: 18,
    text: "Who is entitled to vote in Canadian federal elections?",
    options: [
      "A Canadian citizen, at least 18 years old on voting day, on the voters' list.",
      "A permanent resident, at least 18 years old on voting day, on the voters' list.",
      "Anyone living in Canada for at least 5 years.",
      "Only individuals who pay income tax."
    ],
    correctAnswer: 0,
    category: "How Canadians Govern Themselves",
    explanation: "To vote in a federal election, you must be a Canadian citizen, at least 18 years old on voting day, and on the voters' list.",
    reference: "Discover Canada Guide, Pages 30-31"
  },
  {
    id: 19,
    text: "What are the three levels of government?",
    options: [
      "Executive, Legislative, Judicial.",
      "Federal, Provincial/Territorial, Municipal (local).",
      "Prime Minister, Cabinet, Members of Parliament.",
      "Mayor, Premier, Governor General."
    ],
    correctAnswer: 1,
    category: "Federal Elections",
    explanation: "The three levels of government are Federal, Provincial and Territorial, and Municipal (local) government.",
    reference: "Discover Canada Guide, Page 32"
  },
  {
    id: 20,
    text: "Name two Canadian symbols.",
    options: [
      "The bald eagle and the stars.",
      "The beaver and the maple leaf.",
      "The polar bear and the snowflake.",
      "The rose and the thistle."
    ],
    correctAnswer: 1,
    category: "Canadian Symbols",
    explanation: "The beaver and the maple leaf are prominent Canadian symbols recognized worldwide.",
    reference: "Discover Canada Guide, Pages 38-39"
  }
];
